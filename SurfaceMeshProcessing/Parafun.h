...
public:
	...
	void Tutte();
	void find_bnd_edgePair();

	void Pre_calculate();
	void Pre_calculate_seamless();
	void CM();
	void SLIM();
	...
	int edgepair_num;
	int cuts_num;
	//vector<vector<pair<int, int>>> cuts;
	double seamless_weight;
	...
	vector<vector<edgeR>> cuts;
	vector<edgeR > cutsplus;
	Mesh mesh;
	double Intp_T_Min;
