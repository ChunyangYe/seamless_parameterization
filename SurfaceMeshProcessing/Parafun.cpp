...
Parafun::Parafun(string filename)
{
	...
	find_bnd_edgePair();
	cout << " finish findpair: " << endl;
	position_of_mesh.resize(2 * V_N + 2 * cuts_num);
	negative_grad_norm.resize(2 * V_N + 2 * cuts_num);
}
...
void Parafun::run_cm()
{
	...
	while (iter_num_cur < MAX_ITER_NUM)
	{
		...
		energy_cur = energy_area;
		conv_percent = abs(energy_cur - energy_pre) / energy_pre;
		cout << conv_percent << endl;
		//calc_gradient_norm(position_of_mesh);
		if (conv_percent <= convgence_con_rate)
		{
			cout << "jinru2================" << endl;
			is2 = true;
			seamless_weight = 1000000;
			while (iter_num_cur < MAX_ITER_NUM)
			{
				iter_num_cur++;
				CM();
				...
				if (energy_2 < 1e-5)
				{
					...
					seamless_weight3 = 1e-5;

					while (iter_num_cur < MAX_ITER_NUM)
					{
						seamless_weight3 *= 10;
						seamless_weight = seamless_weight3;

						cout << "seamless_weight==============" << seamless_weight << endl;
	...
	cout << " sum_iter: " << iter_num_cur << endl;
...
void Parafun::find_bnd_edgePair()
{

	string model_str = "E:/testcone/bear/bear.obj";
	ifstream ifs("E:/testcone/bear/bear_landmarks.txt");
	...
	while (getline(ifs, line))
	{
		...
		int landid;
		iss >> landid;
		landmarks.push_back(landid);
	}

	Mesh mesh_close;
	OpenMesh::IO::read_mesh(mesh_close, model_str);

	vector<OpenMesh::Vec3d> land_pos;

	for (auto&var : landmarks)
	{
		land_pos.push_back(mesh_close.point(mesh_close.vertex_handle(var)));
	}

	vector<int> he_bnds;
	vector<int> bnd_vids;
	vector<pair<int, int>> flag_star;
	...
	do
	{
		he_bnds.push_back(he_it.idx());
		bnd_vids.push_back(mesh.from_vertex_handle(he_it).idx());
		OpenMesh::Vec3d vpos = mesh.point(mesh.from_vertex_handle(he_it));
		for (size_t i = 0; i < land_pos.size(); i++)
		{
			if (vpos == land_pos[i])
			{
				flag_star.emplace_back(he_bnds.size() - 1, i);
				break;
			}
		}
		he_it = mesh.next_halfedge_handle(he_it);
	...
	cout << flag_star.size() << "boundary_num: " << boundary_num << endl;
	edgepair_num = boundary_num / 2;
	...
	vector<cut_part> cut_vec(flag_star.size());
	for (size_t i = 1; i < flag_star.size(); i++)
	{
		cut_vec[i - 1].start_pos_id = flag_star[i - 1].second;
		cut_vec[i - 1].end_pos_id = flag_star[i].second;

		cut_vec[i - 1].start_index = flag_star[i - 1].first;
		cut_vec[i - 1].end_index = flag_star[i].first;
	}
	{
		cut_vec[flag_star.size() - 1].start_pos_id = flag_star[flag_star.size() - 1].second;
		cut_vec[flag_star.size() - 1].end_pos_id = flag_star[0].second;

		cut_vec[flag_star.size() - 1].start_index = flag_star[flag_star.size() - 1].first;
		cut_vec[flag_star.size() - 1].end_index = flag_star[0].first;
	}

	cuts_num = cut_vec.size() / 2;
	cuts.reserve(cuts_num);
	vector<int> flag(cut_vec.size(), 0);
	...
	for (size_t i = 0; i < cut_vec.size(); i++)
	{
		if (flag[i] == 1)
			continue;
		flag[i] = 1;
		for (size_t j = 0; j < cut_vec.size(); j++)
		{
			if (flag[j] == 1)
				continue;
			if (cut_vec[i].start_pos_id == cut_vec[j].end_pos_id&&cut_vec[i].end_pos_id == cut_vec[j].start_pos_id)
			{
				flag[j] = 1;

				vector<edgeR>he_pairs;
				int pairs_num = (cut_vec[i].end_index - cut_vec[i].start_index + boundary_num) % boundary_num;
				for (size_t k = 0; k < pairs_num; k++)
				{
					int tmp = cut_vec[i].start_index + k;
					tmp = (tmp >= boundary_num ? tmp - boundary_num : tmp);

					auto v00 = mesh.from_vertex_handle(mesh.halfedge_handle(he_bnds[tmp])).idx();
					auto v01 = mesh.to_vertex_handle(mesh.halfedge_handle(he_bnds[tmp])).idx();
					int he1 = he_bnds[(cut_vec[j].end_index - 1 - k + boundary_num) % boundary_num];
					auto v10 = mesh.to_vertex_handle(mesh.halfedge_handle(he1)).idx();
					auto v11 = mesh.from_vertex_handle(mesh.halfedge_handle(he1)).idx();
					edgeR er;
					er.left = make_pair(v00, v01);
					er.right = make_pair(v10, v11);
					if (v00 == v10)
					{
						er.flag = 2;
					}
					...
					he_pairs.push_back(er);
				}
				cuts.emplace_back(he_pairs);

				{
					cout << "=========================================================" << endl;
					cout << cut_vec[i].start_pos_id << " " << cut_vec[i].end_pos_id << " " << cut_vec[i].start_index << " " << cut_vec[i].end_index << endl;
					cout << cut_vec[j].start_pos_id << " " << cut_vec[j].end_pos_id << " " << cut_vec[j].start_index << " " << cut_vec[j].end_index << endl;
					cout << he_pairs.size() << endl;
					sum_pairs += he_pairs.size();
				}
				break;
			}
		}
...
void Parafun::Pre_calculate_seamless()
{
	source_p00.resize(F_N);
	source_p01.resize(F_N);
	source_p10.resize(F_N);
...
void Parafun::CM()
{
	...
	double h00, h01, h02, h03, h04, h05,
		...
		h55;
	double *position = position_of_mesh.data();
	Eigen::VectorXd eigen_rhs = Eigen::VectorXd::Zero(2 * V_N + 2 * cuts_num);
	...
	for (int i = 0; i < F_N; ++i)
	{
		...
		tripletlist.push_back(T(f0, f0, h00));
		tripletlist.push_back(T(f0, f1, h01));
		tripletlist.push_back(T(f0, f2, h02));
		tripletlist.push_back(T(f0, f0 + V_N, h03));
		tripletlist.push_back(T(f0, f1 + V_N, h04));
		tripletlist.push_back(T(f0, f2 + V_N, h05));

		tripletlist.push_back(T(f1, f0, h01));
		tripletlist.push_back(T(f1, f1, h11));
		tripletlist.push_back(T(f1, f2, h12));
		tripletlist.push_back(T(f1, f0 + V_N, h13));
		tripletlist.push_back(T(f1, f1 + V_N, h14));
		tripletlist.push_back(T(f1, f2 + V_N, h15));


		tripletlist.push_back(T(f2, f0, h02));
		tripletlist.push_back(T(f2, f1, h12));
		tripletlist.push_back(T(f2, f2, h22));
		tripletlist.push_back(T(f2, f0 + V_N, h23));
		tripletlist.push_back(T(f2, f1 + V_N, h24));
		tripletlist.push_back(T(f2, f2 + V_N, h25));

		tripletlist.push_back(T(f0 + V_N, f0, h03));
		tripletlist.push_back(T(f0 + V_N, f1, h13));
		tripletlist.push_back(T(f0 + V_N, f2, h23));
		tripletlist.push_back(T(f0 + V_N, f0 + V_N, h33));
		tripletlist.push_back(T(f0 + V_N, f1 + V_N, h34));
		tripletlist.push_back(T(f0 + V_N, f2 + V_N, h35));


		tripletlist.push_back(T(f1 + V_N, f0, h04));
		tripletlist.push_back(T(f1 + V_N, f1, h14));
		tripletlist.push_back(T(f1 + V_N, f2, h24));
		tripletlist.push_back(T(f1 + V_N, f0 + V_N, h34));
		tripletlist.push_back(T(f1 + V_N, f1 + V_N, h44));
		tripletlist.push_back(T(f1 + V_N, f2 + V_N, h45));

		tripletlist.push_back(T(f2 + V_N, f0, h05));
		tripletlist.push_back(T(f2 + V_N, f1, h15));
		tripletlist.push_back(T(f2 + V_N, f2, h25));
		tripletlist.push_back(T(f2 + V_N, f0 + V_N, h35));
		tripletlist.push_back(T(f2 + V_N, f1 + V_N, h45));
		tripletlist.push_back(T(f2 + V_N, f2 + V_N, h55));



		eigen_rhs[f0] -= (u*a1x0 + v * a2x0);
		eigen_rhs[f1] -= (u*a1x1 + v * a2x1);
		eigen_rhs[f2] -= (u*a1x2 + v * a2x2);
		eigen_rhs[f0 + V_N] -= (u*a1x3 + v * a2x3);
		eigen_rhs[f1 + V_N] -= (u*a1x4 + v * a2x4);
		eigen_rhs[f2 + V_N] -= (u*a1x5 + v * a2x5);
	}
	...
	if (is2)
	{
		for (size_t i = 0; i < cuts.size(); i++)
		{
			for (auto&var : cuts[i])
			{
				if (var.flag == 0)
				{
					int ids_tmp[10] = { var.left.second ,var.left.first ,var.right.second ,var.right.first ,var.left.second + V_N ,
										var.left.first + V_N ,var.right.second + V_N ,var.right.first + V_N ,i + 2 * V_N ,i + 2 * V_N + cuts_num };

					double x0 = position[var.left.second];
					double x1 = position[var.left.first];
					double x2 = position[var.right.second];
					double x3 = position[var.right.first];
					double y0 = position[var.left.second + V_N];
					double y1 = position[var.left.first + V_N];
					double y2 = position[var.right.second + V_N];
					double y3 = position[var.right.first + V_N];
					double ai = position[i + 2 * V_N];
					double bi = position[i + 2 * V_N + cuts_num];

					double r0 = ai * (x1 - x0) + bi * (y1 - y0) - x3 + x2;
					double r1 = ai * (y1 - y0) - bi * (x1 - x0) - y3 + y2;
					...
					gr0 << -ai, ai, 1, -1, -bi, bi, 0, 0, x1 - x0, y1 - y0;
					gr1 << bi, -bi, 0, 0, -ai, ai, 1, -1, y1 - y0, x0 - x1;
					gr = seamless_weight * (gr0 * r0 + gr1 * r1);

					for (size_t j = 0; j < 10; j++)
					{
						eigen_rhs[ids_tmp[j]] -= gr[j];
					}
					for (size_t j = 0; j < 10; j++)
					{
						for (size_t k = 0; k < 10; k++)
						{
							tripletlist.push_back(T(ids_tmp[j], ids_tmp[k], (gr0[j] * gr0[k] + gr1[j] * gr1[k])*seamless_weight));
						}
					}
				...
				{
					int ids_tmp[8] = { var.left.second ,var.left.first ,var.right.first ,var.left.second + V_N ,
					var.left.first + V_N ,var.right.first + V_N ,i + 2 * V_N ,i + 2 * V_N + cuts_num };

					double x0 = position[var.left.second];
					double x1 = position[var.left.first];
					double x2 = position[var.right.first];
					double y0 = position[var.left.second + V_N];
					double y1 = position[var.left.first + V_N];
					double y2 = position[var.right.first + V_N];
					double ai = position[i + 2 * V_N];
					double bi = position[i + 2 * V_N + cuts_num];

					double r0 = ai * (x1 - x0) + bi * (y1 - y0) - x2 + x0;
					double r1 = ai * (y1 - y0) - bi * (x1 - x0) - y2 + y0;

					VectorXd gr0, gr1, gr;
					gr0.resize(8);
					gr1.resize(8);
					gr.resize(8);
					gr0 << 1.0 - ai, ai, -1, -bi, bi, 0, x1 - x0, y1 - y0;
					gr1 << bi, -bi, 0, 1.0 - ai, ai, -1, y1 - y0, x0 - x1;
					gr = seamless_weight * (gr0 * r0 + gr1 * r1);
				...
				{
					int ids_tmp[8] = { var.left.first ,var.left.second ,var.right.second ,var.left.first + V_N ,var.left.second + V_N ,
					var.right.second + V_N ,i + 2 * V_N ,i + 2 * V_N + cuts_num };

					double x0 = position[var.left.first];
					double x1 = position[var.left.second];
					double x2 = position[var.right.second];
					double y0 = position[var.left.first + V_N];
					double y1 = position[var.left.second + V_N];
					double y2 = position[var.right.second + V_N];
					double ai = position[i + 2 * V_N];
					double bi = position[i + 2 * V_N + cuts_num];
			...
			if (is3)
			{
				int ids_tmp[2] = { i + 2 * V_N ,i + 2 * V_N + cuts_num };
				double a_i = position[i + 2 * V_N];
				double b_i = position[i + 2 * V_N + cuts_num];
				double r0 = a_i * a_i + b_i*b_i - 1;
				double r1 = a_i * b_i;
				...
				gr0 << 2*a_i, 2*b_i;
				gr1 << b_i, a_i;
				gr = seamless_weight3 * (gr0 * r0 + gr1 * r1);
				...
				for (size_t j = 0; j < 2; j++)
				{
					...
					{
						tripletlist.push_back(T(ids_tmp[j], ids_tmp[k], (gr0[j] * gr0[k] + gr1[j] * gr1[k])*seamless_weight3));
					}
				}
			}
	...
	else
	{
		...
		{
			tripletlist.push_back(T(i + 2 * V_N, i + 2 * V_N, 1.0));
			tripletlist.push_back(T(i + 2 * V_N + cuts_num, i + 2 * V_N + cuts_num, 1.0));

		}
	}

	Eigen::SparseMatrix<double> eigen_H;
	eigen_H.resize(2 * V_N + 2 * cuts_num, 2 * V_N + 2 * cuts_num);


	// Eigen solver
	eigen_H.setFromTriplets(tripletlist.begin(), tripletlist.end());
	std::cout << "1111111111111111\n";

	Eigen::SparseLU<Eigen::SparseMatrix<double>> solver;
	solver.compute(eigen_H); // Pre-decomposition

	if (solver.info() != Eigen::Success)
	{
		std::cout << "Decompose Matrix error\n";
	}
	Eigen::VectorXd result_d = solver.solve(eigen_rhs);
	std::cout << "2222222222222222222\n";

	VectorXd negative_grad(2 * V_N + 2 * cuts_num), d(2 * V_N + 2 * cuts_num);
	for (int i = 0; i < 2 * V_N + 2 * cuts_num; i++)
	{
		negative_grad(i) = eigen_rhs[i];
		d(i) = result_d[i];
	}
...
//	if (is2)
//	{
//		for (size_t i = 0; i < cutsplus.size(); i++)
//		{
//			auto&var = cutsplus[i];
//			if (var.flag == 0)
//			{
...
//				double a_i = position[i + 2 * V_N];
//				double b_i = position[i + 2 * V_N + cuts_num];
//				double g_a = 4 * (a_i*a_i + b_i * b_i - 1)*a_i + 2 * a_i*b_i*b_i;
//				double g_b = 4 * (a_i*a_i + b_i * b_i - 1)*b_i + 2 * a_i*a_i*b_i;
//				double h_00 = 4 * (3 * a_i*a_i + b_i * b_i - 1) + 2 * b_i*b_i;
//				double h_01 = 12 * a_i*b_i;
//				double h_11 = 4 * (a_i*a_i + 3 * b_i*b_i - 1) + 2 * a_i*a_i;
//				eigen_rhs[i + 2 * V_N] -= g_a * seamless_weight3;
//				eigen_rhs[i + 2 * V_N + cuts_num] -= g_b * seamless_weight3;
//
//				tripletlist.push_back(T(i + 2 * V_N, i + 2 * V_N, h_00* seamless_weight3));
//				tripletlist.push_back(T(i + 2 * V_N, i + 2 * V_N + cuts_num, h_01* seamless_weight3));
//				tripletlist.push_back(T(i + 2 * V_N + cuts_num, i + 2 * V_N, h_01* seamless_weight3));
//				tripletlist.push_back(T(i + 2 * V_N + cuts_num, i + 2 * V_N + cuts_num, h_11* seamless_weight3));
//
//			}
//
//		}
...
void Parafun::Energy(const VectorXd &position, double &energyupdate)
{
	...
	if (is2)
	{
		for (size_t i = 0; i < cuts.size(); i++)
		{
			for (auto&var : cuts[i])
			{
				double x0 = pos[var.left.second];
				double x1 = pos[var.left.first];
				double x2 = pos[var.right.second];
				double x3 = pos[var.right.first];
				double y0 = pos[var.left.second + V_N];
				double y1 = pos[var.left.first + V_N];
				double y2 = pos[var.right.second + V_N];
				double y3 = pos[var.right.first + V_N];
				double ai = pos[i + 2 * V_N];
				double bi = pos[i + 2 * V_N + cuts_num];

				double r0 = ai * (x1 - x0) + bi * (y1 - y0) - x3 + x2;
				double r1 = ai * (y1 - y0) - bi * (x1 - x0) - y3 + y2;
				energy += seamless_weight * 0.5*(r0*r0 + r1 * r1);
			}

			if (is3)
			{
				double a_i = pos[i + 2 * V_N];
				double b_i = pos[i + 2 * V_N + cuts_num];

				double a2b2 = (a_i*a_i + b_i * b_i - 1.0);
				energy += seamless_weight3 * 0.5*(a2b2*a2b2 + a_i * a_i * b_i * b_i);
			}
		}
	}
	...
	if (is2)
	{
		...
		for (size_t i = 0; i < cuts.size(); i++)
		{
			for (auto&var : cuts[i])
			{
				...
				double r0 = ai * (x1 - x0) + bi * (y1 - y0) - x3 + x2;
				double r1 = ai * (y1 - y0) - bi * (x1 - x0) - y3 + y2;
				double e2 = 0.5*(r0*r0 + r1 * r1);
				end_e_area += seamless_weight * e2;
				energy_2 += e2;
			}
			...
			if (is3)
			{
				...
				double a2b2 = (a_i*a_i + b_i * b_i - 1.0);
				double e3 = 0.5*(a2b2*a2b2 + a_i * a_i * b_i * b_i);
				end_e_area += seamless_weight3 * e3;
				energy_3 += e3;
			}
		}
		cout << "------------------------------------------" << endl;
		...
		cout << "energy_2===== " << energy_2 << "; energy_3===== " << energy_3 << endl;
		cout << "energy_sum===== " << end_e_area << endl;
	}
